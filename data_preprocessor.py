...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self,
        ...
        if voxel:
            ...
            voxel_layer_hight=dict(
                max_num_points=10,
                point_cloud_range=[0, -39.68, -3, 69.12, 39.68, 1],
                # point_cloud_range=[0, -40, -3, 70.4, 40, 1],
                voxel_size=[0.16, 0.16, 0.0625],
                max_voxels=(16000, 40000))
            self.voxel_layer_hight = VoxelizationByGridShape(**voxel_layer_hight)
    ...
    def voxelize(self, points: List[Tensor],
        ...
        if self.voxel_type == 'hard':
            ...
            voxels_height, coors_height, num_points_height = [], [], []
            for i, res in enumerate(points):
                ...
                res_voxels_h, res_coors_h, res_num_points_h = self.voxel_layer_hight(res)
                res_coors_h = F.pad(res_coors_h, (1, 0), mode='constant', value=i)
                voxels_height.append(res_voxels_h)
                coors_height.append(res_coors_h)
                num_points_height.append(res_num_points_h)
            ...
            voxels_height = torch.cat(voxels_height, dim=0)
            coors_height = torch.cat(coors_height, dim=0)
            num_points_height = torch.cat(num_points_height, dim=0)
            voxel_dict['num_points_h'] = num_points_height
        ...
        voxel_dict['voxels_h'] = voxels_height
        voxel_dict['coors_h'] = coors_height
